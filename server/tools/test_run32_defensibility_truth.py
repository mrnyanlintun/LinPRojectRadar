...
# subject is A1.1,A3.4,A4.1,B2.9,B4.1,B4.2 -- 6 module ids removed from the registry at Run 96 or Run 97 and no module
...
print("RETIRED: test_run32_defensibility_truth.py measures A1.1,A3.4,A4.1,B2.9,B4.1,B4.2, removed at Run 96/97 (88e6ca0); excluded from qualification coverage "
...
for rel in ("assets/js/categories.js", "assets/js/taxonomy.js"):
    ...
    m = re.search(r"module_id: 'B4\.7', name: '([^']*)', method_class: '([^']*)'", txt)
    if _B47_RETIRED:
        check(m is None and "module_id: 'B4.7'" not in txt,
    ...
    _names = dict(re.findall(r"module_id: '([A-D][0-9]+\.[0-9]+)', name: '([^']*)'", txt))
    _classes = dict(re.findall(
        r"module_id: '([A-D][0-9]+\.[0-9]+)', name: '[^']*', method_class: '([^']*)'", txt))
