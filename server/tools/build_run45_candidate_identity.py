#!/usr/bin/env python3
"""
RUN 45. THE SUCCESSOR FREEZE CANDIDATE IDENTITY.

Writes research/freeze/run44_freeze_candidate_identity.json in exactly the shape Run 41 wrote its
own, so the freeze gate's B01 blocker can recompute every digest from the tree and compare.

The Run-44 identity is NOT rewritten. It stays as that release wrote it and remains the identity
of the v29 predecessor; this file is its successor and names it as its parent.
...
globs, and the analytical layer - are re-derived from the filesystem, because Run 45 adds a suite
and a frozen list would silently stop covering it.
...
PARENT_IDENTITY = FREEZE / "run44_freeze_candidate_identity.json"
...
def main() -> int:
    ...
    out: dict = {
        ...
        "supersession_reason": (
            "Run 45 closes the period-scoping fall-through Run 44 measured, on the owner's "
            "ruling of 2026-08-22 signing off the canonical field classification. Every "
            "observation was scoped to the period its document was uploaded into, which is "
            "right for a fact about one reporting period and wrong for a fact about the "
            "project: a contract uploaded at period 1 was invisible from period 2 on, so `bac` "
            "fell through to a pay application's weaker restatement -- 4,463,290 where the "
            "contract said 5,874,620 -- and `baselineContractSum` INVERTED its own declared "
            "precedence, a change order's account of the original beating the contract that "
            "established it. Fields are now retrieved by canonical kind: 13 IDENTITY fields "
            "return the latest value at or before the period being computed with declared "
            "document-type precedence holding across the carry-forward, 62 PERIOD fields "
            "retrieve exactly as before, and 2 are recorded UNDETERMINED and retrieved as "
            "period fields, which is the unchanged behaviour. WHAT A MODULE IS GIVEN is "
            "executable behaviour, so the freeze is SUPERSEDED rather than amended. NO FORMULA, "
            "BAND, THRESHOLD OR POPULATION MOVED: voting is still exactly A1.7 and A1.8, 63 "
            "modules in service of 101 registered. On the repository fixtures exactly three "
            "modules move, all attributable to an identity field newly visible: A1.7 and A1.8 "
            "to `bac`, A3.2 to `originalContingency`; the two control corpora are byte-"
            "identical before and after. NO SEQUENCE-BEARING FILE MOVED and no user-facing "
            "control was added, moved or removed. The v29 candidate, its identity, its gate and "
            "its release records are preserved unchanged as the historical evidence for "
            "anything computed under v29."),
    ...
    merge = list(parent["evidence_provenance_digest"]["members"])
