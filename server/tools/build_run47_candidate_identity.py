#!/usr/bin/env python3
"""
RUN 47. THE SUCCESSOR FREEZE CANDIDATE IDENTITY.

Writes research/freeze/run47_freeze_candidate_identity.json in exactly the shape Run 41 wrote its
own, so the freeze gate's B01 blocker can recompute every digest from the tree and compare.

The Run-45 identity is NOT rewritten. It stays as that release wrote it and remains the identity
of the v30 predecessor; this file is its successor and names it as its parent. (Run 46 was
report-only and minted nothing, so v30 is the immediate predecessor.)
...
PARENT_IDENTITY = FREEZE / "run45_freeze_candidate_identity.json"
...
def main() -> int:
    ...
    out: dict = {
        ...
        "supersession_reason": (
            "Run 47 adds the EVM consistency check, on the owner's four rulings of 2026-08-22. "
            "Where ONE document states both a value and the percentage that determines it "
            "against a known budget at completion, the implied value is computed and the two "
            "are compared; a relative difference above 2 per cent, measured against the implied "
            "value, is reported as a finding on the Executive Brief and beside the "
            "recommendation. Two relations are checked: planned value against budget at "
            "completion times planned percent complete, and earned value against budget at "
            "completion times actual percent complete. THE DOCUMENT TAKES PRECEDENCE AND "
            "NOTHING IS DERIVED INTO STORAGE: no figure is corrected, substituted, clamped or "
            "bounded, `pv` is absent from BOUNDED_MAX_SI_FIELDS and stays absent, and the check "
            "runs on the READ path from the stored row, so no stored figure can change. A "
            "finding carries no band, no colour and no severity, casts no vote and is read by "
            "no module: the full served census with and without a disagreement present is "
            "identical, asserted by test_run47_evm_consistency.py section 6. NO FORMULA, BAND, "
            "THRESHOLD OR POPULATION MOVED: voting is still exactly A1.7 and A1.8, 63 modules "
            "in service of 101 registered. NO SEQUENCE-BEARING FILE MOVED and no user-facing "
            "control was added, moved or removed. The retired 'Cat N' labels in "
            "`BRIEF_CAT_LABEL` were corrected to groups and purposes under "
            "NAMING_AUTHORITY.md:96. The v30 candidate, its identity, its gate and its release "
            "records are preserved unchanged as the historical evidence for anything computed "
            "under v30."),
        "simulation_version": "sim-2026.08-v31",
        "participant_package": "og-participant-2026.08-v15",
    ...
    # RUN 47. ONE NAMED ADDITION: the consistency check is built on the per-field provenance
    # record this group already measures, and reads `sources[field].documentId` to establish
    # that two figures came from one document. Added rather than globbed, so a future file in
    # app/ does not enter this group silently.
    ...
    target = FREEZE / "run47_freeze_candidate_identity.json"
