...
RELEASE_ID = "OPUS-GUBERNATIO-RESEARCH-INSTRUMENT-2026-08-16-RUN30-FINAL-CLOSURE-V16-2"
PARENT_ID = "OPUS-GUBERNATIO-RESEARCH-INSTRUMENT-2026-08-16-RUN30-CLOSURE-V16-1"
STAGE1 = ROOT / "research" / "freeze" / "RUN30_FINAL_CLOSURE_FREEZE_2026-08-16.json"
STAGE2 = STAGE1.with_suffix(".sha256")
PARENT_FREEZE = "research/freeze/RUN30_CLOSURE_FREEZE_2026-08-16.json"
PARENT_FREEZE_STAGE2 = "research/freeze/RUN30_CLOSURE_FREEZE_2026-08-16.sha256"
...
RUN30_CHANGED = {
    "server/app/simulation/lineage.py": (
        "declared by Run 29",
        "THE FOUR LINEAGE STATES. Run 30's closure removed eleven Category-7 declarations whose "
        "content had become false and deliberately did not replace them with invented "
        "independent bodies. That left the right facts in the wrong representation: a row with "
        "no declaration was indistinguishable from one whose independence had been established, "
        "because both carried nothing. LINEAGE_ESTABLISHED_INDEPENDENT, "
        "LINEAGE_ESTABLISHED_DEPENDENT, LINEAGE_UNRESOLVED and LINEAGE_NOT_APPLICABLE name the "
        "state, and `lineage_status` DERIVES it from the shipped declaration table so it cannot "
        "be asserted into existence. Nothing here is a new semantic: fusion.py has implemented "
        "the unresolved state since FUSION.1 in Run 20 cycle 9. What is added is a name for it."),
    "server/app/simulation/models_cat7.py": (
        "declared by Run 30 as new production code",
        "every Category-7 ledger row, computed or abstaining, now carries its lineage state, "
        "whether independence is established, the evidence body only where one is established, "
        "and its source provenance SEPARATELY from all three -- because a row may know exactly "
        "where its structure came from and still have unresolved independence."),
}
...
def build() -> None:
    ...
    doc = {
        ...
        "generated_by": "server/tools/build_run30_final_closure_freeze.py",
        "reason":
        ...
        "final_commit": None,
        "starting_commit": "b7709cfb780eabbc0f58d2d3bc029e331e92c32d",
        ...
        "production_surface": {
            ...
            "file_count": len(prod),
            "manifest_file": "code_audit/run30_final_closure_production_tree.sha256",
            "manifest_sha256": pt.manifest_sha256(),
            "parent_manifest_file": "code_audit/run30_closure_production_tree.sha256",
            "parent_manifest_sha256": hashlib.sha256(
                (ROOT / "code_audit"
                 / "run30_closure_production_tree.sha256").read_bytes()).hexdigest(),
