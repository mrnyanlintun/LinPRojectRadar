#!/usr/bin/env python3
"""
RUN 52. THE SUCCESSOR FREEZE RELEASE RECORDS.
...
PREDECESSOR_RECORD = FREEZE / "RUN51_SUCCESSOR_FREEZE_RECORD.json"
PREDECESSOR_CHECKSUMS = FREEZE / "RUN51_SUCCESSOR_FREEZE_CHECKSUMS.csv"
IDENTITY = FREEZE / "run52_freeze_candidate_identity.json"
GATE = FREEZE / "run52_successor_freeze_gate.csv"
BEHAVIOUR = FREEZE / "run52_candidate_behaviour_digest.json"
...
def main() -> int:
    ...
    for extra in ("research/freeze/run52_freeze_candidate_identity.json",
        ...
        rows.append({"path": extra, "sha256": sha(p), "git_object": git_object(p),
                     "tracked": "TRACKED", "moved_since_v32": "ADDED_BY_RUN52"})

    out_sums = FREEZE / "RUN52_SUCCESSOR_FREEZE_CHECKSUMS.csv"
    ...
    rec = {
        ...
        "label": "Opus Gubernatio research instrument, Run-52 successor freeze (sim-2026.08-v35)",
        "created": datetime.date(2026, 8, 22).isoformat(),
        "authority": (
            "The owner's FOUR RULINGS of 2026-08-23 in the Run 52 order, section 3. "
            "1. REMOVE THE OPEN BUTTON FROM THE PROJECT LIST -- NOT CARRIED, SURFACE STOPPED "
            "UNDER SECTION 8.1. The ruling's premise was that Manage and Open lead to the same "
            "project detail page. Driven in a real browser they do not: Manage opens an inline "
            "admin accordion under its own row and never leaves the portfolio page, while Open "
            "is the ONLY route from the project list to the project detail page, so removing it "
            "would have removed that route. assets/js/app.js did not move. "
            "2. REMOVE THE DEAD 'see Health' BUTTON -- CARRIED. Its handler called "
            "window.LinIngest.openHealthModal(), which exists nowhere in the repository, so "
            "clicking it did nothing. The button and its [data-goto-health] handler are gone; "
            "the anomaly sentence it sat beside is unchanged and still renders. "
            "3. ONE NAME FOR THE MODULE IDENTIFIER ON BOTH SIDES OF THE WIRE, `module_id` -- "
            "CARRIED, with two named stops. The authority, the generator, both regenerated "
            "mirrors and every client consumer move to the server's existing name. The CATEGORY "
            "identifier keeps `key`: a category is not a module. The registry map's "
            "`new_id`/`old_id` pair is STOPPED under section 8.2 and reported. The deep-dive "
            "methods-comparison array's local `num` is STOPPED under section 8.2: it is a "
            "method ordinal, not a registry module identifier. "
            "4. IDENTIFIERS ON SCREEN ARE NOT TOUCHED -- OBEYED AS THE REVERSAL IT IS. NO "
            "NAMING SWEEP WAS RUN; nothing was stripped from rendered text and nothing was "
            "restored. NO STORED FIGURE CHANGED, no band, status, colour or posture changes, "
            "and the behaviour digest is unchanged."),
        ...
        "release_content_digest_method": (
            "sha256 over RUN52_SUCCESSOR_FREEZE_CHECKSUMS.csv, which content-addresses every "
...
"""
    ...
    print("wrote RUN52_SUCCESSOR_FREEZE_REPORT.md")
