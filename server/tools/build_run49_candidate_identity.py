#!/usr/bin/env python3
"""
RUN 49. THE SUCCESSOR FREEZE CANDIDATE IDENTITY.

Writes research/freeze/run49_freeze_candidate_identity.json in exactly the shape Run 41 wrote its
...
Usage: python tools/build_run49_candidate_identity.py [--candidate <commit>]
"""
...
PARENT_IDENTITY = FREEZE / "run48_freeze_candidate_identity.json"
...
def main() -> int:
    ...
    out: dict = {
        ...
        "supersession_reason": (
            "Run 49 finishes the naming correction Run 48's own second sweep left unfinished, "
            "on the owner's five rulings of 2026-08-22. RULING 1: every surviving RENDERED "
            "instance of the retired 'Cat N' scheme is corrected in assets/js/deepdive.js -- the "
            "ten collapsible group headers, the Signal Stack banner, the Dempster-Shafer "
            "metric-box label, the synthesis comparison heading, its note, its three confidence "
            "sentences, the comparison table's per-row module prefix and its 'Agrees with M09' "
            "column header, and the Portfolio Health flyout's module headings -- together with "
            "the ampersand and the en dashes those sentences carried, and the sentence in "
            "detail.js that reached the executive brief's model. RULING 2: the ampersand at "
            "detail.js's document section title becomes the word 'and'. RULING 3: the deep-dive "
            "panel label map is EXTENDED from nineteen keys to all seventy-seven the call sites "
            "actually pass, so each panel names its own category's purpose instead of the one "
            "neutral fallback Run 48 collapsed roughly sixty panels onto; no key is left on the "
            "fallback and none required deciding a category the module's own identity does not "
            "determine. RULING 4: the three inert `period: 1` literals in decision-ui.js are "
            "LEFT IN PLACE and the file now records at each of them that documents._resolve_"
            "period derives the period from the research assignment and ignores the value. "
            "RULING 5: no panel states its reporting period and no control was added anywhere. "
            "NOTHING IS COMPUTED DIFFERENTLY AND NO STORED FIGURE MOVED: no formula, band, "
            "threshold, calibration, abstention rule or population moved, voting is still "
            "exactly A1.7 and A1.8, and 63 modules are in service of 101 registered. NO MODULE "
            "BUCKETS DIFFERENTLY: CAT_NUM_FROM_MODULE, the grouping map Run 48 separated from "
            "the displayed text for exactly this purpose, is byte-identical to v32. TWO "
            "SEQUENCE-BEARING FILES MOVED, assets/js/deepdive.js and assets/js/decision-ui.js, "
            "each on its own ruling's authority and each carrying its own named exception record "
            "in participant_packages.py; the other four are byte-identical. The v32 candidate, "
            "its identity, its gate and its release records are preserved unchanged as the "
            "historical evidence for anything computed under v32."),
        "simulation_version": "sim-2026.08-v33",
        "participant_package": "og-participant-2026.08-v18",
    ...
    target = FREEZE / "run49_freeze_candidate_identity.json"
