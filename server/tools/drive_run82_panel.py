#!/usr/bin/env python3
"""
RUN 82. THE CATEGORY PANEL, VERIFIED IN A BROWSER, BEFORE AND AFTER PART A.

Derived from drive_run81_panel.py, which it does not replace. What it adds over Run 81:

  A1  the HEADLINE -- "n of N produced a status" -- read from the DOM, per category and for
      the project, and cross-checked against the module roster and the stored readings.
  A2  the COUNT LINE's visible words, so "abstained" can be proved gone and the machine-readable
      data-count attributes proved UNCHANGED.
  A3  the PROGRESS LINE's document position relative to the Process-all control, measured by
      compareDocumentPosition and by bounding box, not asserted.
  A4  the progress line's TEXT after a press, so "Calling" can be proved gone.
  A5  a sha256 over every explanation string in the panel, so Part A can be proved to have left
      the owner's standing ruling untouched.

Run this ONCE on the committed tree (BEFORE) and once after the edit (AFTER), with
R82_OUT set to two different files, and diff them.
...
with Session() as s:
    ...
    if row is None:
        s.add(Participant(pseudonymous_code=f"R82-ADMIN-{STAMP}", role="ResearchAdmin",
...
created = post({"action": "adminparticipantcreate", "session_token": admin,
                "pseudonymous_code": f"R82-PM-{STAMP}", "role": "Participant",
...
say("=" * 100); say(f"RUN 82  project={PID}  DATABASE_URL={os.environ.get('DATABASE_URL')}")
say("=" * 100)
...
READ = r"""() => {
  ...
  const actions = document.querySelector('.dcat-actions');
  const line = document.querySelector('.dcat-status-line');
  const headline = document.querySelector('.dcat-headline');
  /* A3. DOCUMENT ORDER, MEASURED. compareDocumentPosition returns 4 when `line` FOLLOWS
     `actions` in document order and 2 when it precedes it; the bounding boxes are read as
     well so a hidden or zero-height line cannot pass on document order alone. */
  ...
  const list = document.querySelector('.dcat-list');
  const progress = {
    present: !!line,
    role: line ? line.getAttribute('role') : null,
    text: line ? line.textContent : null,
    box: bx(line),
    actionsBox: bx(actions),
    listBox: bx(list),
    /* 4 = line comes AFTER actions, 2 = line comes BEFORE actions */
    posVsActions: (line && actions) ? (actions.compareDocumentPosition(line) & 6) : null,
    posVsList: (line && list) ? (list.compareDocumentPosition(line) & 6) : null,
    aboveList: (line && list) ? (bx(line).y < bx(list).y) : null
  };
  const head = {
    present: !!headline,
    text: headline ? headline.textContent : null,
    produced: headline ? headline.getAttribute('data-produced') : null,
    ofTotal: headline ? headline.getAttribute('data-of') : null,
    box: bx(headline),
    /* the headline must come FIRST inside the panel */
    firstChildClass: panel && panel.firstElementChild
        ? (panel.firstElementChild.className||'').toString() : null
  };
  const perCat = Array.from(document.querySelectorAll('.dcat-row')).map(r => ({
    cat: r.getAttribute('data-category'),
    produced: r.querySelector('.dcat-produced')
        ? r.querySelector('.dcat-produced').textContent : null,
    producedN: r.querySelector('.dcat-produced')
        ? r.querySelector('.dcat-produced').getAttribute('data-produced') : null,
    producedOf: r.querySelector('.dcat-produced')
        ? r.querySelector('.dcat-produced').getAttribute('data-of') : null
  }));
  /* A2. The count line: its visible words AND its machine keys, separately. */
  const countCells = Array.from(document.querySelectorAll('.dcat-n')).map(n => ({
    key: n.getAttribute('data-count'), text: n.textContent
  }));
  return { progress, head, perCat, countCells,
           sections, rows, mods, reasons, notes, hint,
...
with sync_playwright() as pw:
    ...
    # ---- A4. PRESS A CATEGORY AND READ THE PROGRESS LINE'S FIRST WORDS ----------------
    # The line is set synchronously at the top of specCall, before the await, so a short
    # poll catches the in-flight text. This is the word the owner is objecting to.
    inflight = None
    try:
        pg.evaluate("""() => { const b = document.querySelector('.dcat-call:not([disabled])');
                               if (b) b.click(); }""")
        for _ in range(60):
            t = pg.evaluate("() => { const l=document.querySelector('.dcat-status-line');"
        ...
        inflight = f"<press failed: {exc}>"
    pg.wait_for_timeout(3000)
    AFTER_PRESS = pg.evaluate(READ)

    say("-" * 100)
    say("RUN 82, PART A -- WHAT THE PANEL SAYS, READ FROM THE RENDERED DOM")
    say(f"A1 headline present: {R['head']['present']}")
    say(f"   headline text   : {R['head']['text']!r}")
    say(f"   data-produced={R['head']['produced']!r} data-of={R['head']['ofTotal']!r} "
        f"box={R['head']['box']}")
    say(f"   panel's first element child: {R['head']['firstChildClass']!r}")
    say("   PER-CATEGORY produced-out-of cells:")
    for c in R["perCat"]:
        say(f"     {str(c['cat']):<4} {str(c['produced'])!r:<24} "
            f"data-produced={c['producedN']} data-of={c['producedOf']}")
    say("")
    say("A2 count line -- VISIBLE WORDS and MACHINE KEYS, separately:")
    for c in R["countCells"]:
        say(f"     data-count={str(c['key']):<13} text={c['text']!r}")
    vis = sorted({c['text'].split(' ',1)[1] for c in R['countCells'] if c['text'] and ' ' in c['text']})
    keys = sorted({str(c['key']) for c in R['countCells']})
    say(f"   distinct visible words: {vis}")
    say(f"   distinct data-count keys: {keys}")
    say(f"   the word 'abstained' visible anywhere in the count line: "
        f"{any('abstained' in (c['text'] or '') for c in R['countCells'])}")
    say("")
    say("A3 progress line placement, MEASURED:")
    pr = R["progress"]
    say(f"   present={pr['present']} role={pr['role']!r}")
    say(f"   compareDocumentPosition(actions -> line) = {pr['posVsActions']} "
        f"(4 = line AFTER actions, 2 = line BEFORE actions)")
    say(f"   compareDocumentPosition(list -> line)    = {pr['posVsList']} "
        f"(4 = line AFTER the eleven rows, 2 = line BEFORE them)")
    say(f"   boxes: actions={pr['actionsBox']}  line={pr['box']}  list={pr['listBox']}")
    say(f"   line is ABOVE the eleven rows on screen: {pr['aboveList']}")
    say("")
    say("A4 the progress line's text while a category call is in flight:")
    say(f"   {inflight!r}")
    say(f"   contains the word 'Calling': {'Calling' in str(inflight)}")
    say(f"   text after the call settled: {AFTER_PRESS['progress']['text']!r}")
    # NON-VACUITY. The empty line has height 0 and shares a y with the list, so a box
    # comparison on an UNPRESSED panel is degenerate and would read the same either way.
    # This is the same box comparison taken once the line CARRIES TEXT and has real height.
    ap = AFTER_PRESS["progress"]
    say(f"   with text: line box={ap['box']}  list box={ap['listBox']}  "
        f"actions box={ap['actionsBox']}")
    say(f"   line has real height: {bool(ap['box'] and ap['box']['h'] > 0)}   "
        f"line is ABOVE the eleven rows on screen: {ap['aboveList']}")
    say("")
    say("A5 EXPLANATION TEXT, sha256 over every explanation string in the panel:")
    blob = json.dumps({"reasons": R["reasons"], "notes": R["notes"], "hint": R["hint"]},
                      sort_keys=True, ensure_ascii=False)
    say(f"   strings: {len(R['reasons'])} module reasons, {len(R['notes'])} notes, 1 hint")
    say(f"   sha256  : {hashlib.sha256(blob.encode('utf-8')).hexdigest()}")
    ...
    OUT = pathlib.Path(os.environ.get("R82_OUT", "/tmp/r82.json"))
    OUT.write_text(json.dumps({
        "explanationSha256": hashlib.sha256(blob.encode("utf-8")).hexdigest(),
        "head": R["head"], "perCat": R["perCat"], "countCells": R["countCells"],
        "progress": R["progress"], "inflight": inflight,
