#!/usr/bin/env python3
"""
RUN 62. THE SUCCESSOR FREEZE RELEASE RECORDS.
...
PREDECESSOR_RECORD = FREEZE / "RUN59_SUCCESSOR_FREEZE_RECORD.json"
PREDECESSOR_CHECKSUMS = FREEZE / "RUN59_SUCCESSOR_FREEZE_CHECKSUMS.csv"
IDENTITY = FREEZE / "run62_freeze_candidate_identity.json"
GATE = FREEZE / "run62_successor_freeze_gate.csv"
BEHAVIOUR = FREEZE / "run62_candidate_behaviour_digest.json"
...
def main() -> int:
    ...
    for r in pred_rows:
        ...
        if not p.is_file():
            ...
            deleted.append(r["path"])
            rows.append({"path": r["path"], "sha256": "", "git_object": "",
                         "tracked": r["tracked"], "moved_since_v32": "DELETED_BY_RUN62"})
    ...
    for extra in ("research/freeze/run62_freeze_candidate_identity.json",
        ...
        rows.append({"path": extra, "sha256": sha(p), "git_object": git_object(p),
                     "tracked": "TRACKED", "moved_since_v32": "ADDED_BY_RUN62"})

    out_sums = FREEZE / "RUN62_SUCCESSOR_FREEZE_CHECKSUMS.csv"
    ...
    (artifact_out(FREEZE / "RUN62_SUCCESSOR_FREEZE_RECORD.json")).write_text(
...
"""
    (artifact_out(FREEZE / "RUN62_SUCCESSOR_FREEZE_REPORT.md")).write_text(report, encoding="utf-8")
