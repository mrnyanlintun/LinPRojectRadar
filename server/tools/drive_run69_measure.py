...
LABEL = sys.argv[1] if len(sys.argv) > 1 else "run69"
OUT = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path("run69_capture.json")
...
DOCS = [
    ...
    ("co2", 2, "change_order",
     {"change_order_date": END[2], "document_date": END[2],
      "modifications_json": [
...
with Session() as s:
    ...
    if row is None:
        s.add(Participant(pseudonymous_code="R69-ADMIN", role="ResearchAdmin",
...
created = post({"action": "adminparticipantcreate", "session_token": admin,
                "pseudonymous_code": f"R69-PM-{int(time.time())}", "role": "Participant",
...
if os.environ.get("RUN69_FIXTURE_ONLY"):
