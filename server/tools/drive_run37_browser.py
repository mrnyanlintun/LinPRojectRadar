#!/usr/bin/env python3
"""
RUN 37, SECTION 7. THE FROZEN PARTICIPANT PACKAGE, RE-QUALIFIED IN A REAL BROWSER.

NOT INHERITED. Section 7 forbids taking Run 36's PASS results on trust, so every surface is
driven again against the candidate tree. This driver CHANGES NOTHING: it authenticates, walks the
controlled sequence, and records what it observes. If a participant-facing executable byte needed
to change, the correct answer is FINAL_FREEZE_BLOCKED and a successor candidate.
...
r12.ADMIN = "r37-browser-admin"
r12.PRJ = ["PRJ-R37B-EV-1", "PRJ-R37B-EV-2"]
BASE = r12.BASE
post = r12.post
...
def main() -> int:
    ...
    print("=" * 94)
    print("RUN 37 SECTION 7. THE FROZEN PARTICIPANT PACKAGE, RE-QUALIFIED IN A REAL BROWSER")
    print("=" * 94)
    ctx = r12.provision()
    tok = ctx["token"]
    ...
    out = ROOT / "code_audit" / "run37_browser_qualification.csv"
