#!/usr/bin/env python3
"""
RUN 51. THE SUCCESSOR FREEZE CANDIDATE IDENTITY.

Writes research/freeze/run51_freeze_candidate_identity.json in exactly the shape Run 41 wrote its
own, so the freeze gate's B01 blocker can recompute every digest from the tree and compare.

The Run-49 identity is NOT rewritten. It stays as that release wrote it and remains the identity
of the v33 predecessor; this file is its successor and names it as its parent.
...
Usage: python tools/build_run51_candidate_identity.py [--candidate <commit>]
"""
...
PARENT_IDENTITY = FREEZE / "run49_freeze_candidate_identity.json"
...
def main() -> int:
    ...
    out: dict = {
        ...
        "supersession_reason": (
            "Run 51 delivers the six rulings Run 50 stopped on, on the owner's order of "
            "2026-08-22. RULING 1: the Portfolio Health flyout is DELETED -- renderCat8Health, "
            "CAT8_MODULES, cat8HealthData, cat8HealthDataFromLive, isSnapshotStale, cat8Retired "
            "and the three buttons inside them. It rendered nowhere and had no caller anywhere "
            "in the served application, and an unreachable button is not a control. RULING 2: "
            "the taxonomy's primary key is SEPARATED FROM THE LABEL and renamed from `num` to "
            "`key` across the authority, both generated mirrors and every consumer; the key "
            "dispatches and what renders is a name that carries no identifier, so the Signal "
            "Ledger's 63 module chips and 11 category chips, the action plan's trigger lines, "
            "the executive brief's category list, the Signal Flow tooltip and the exported "
            "workbook's identifier columns are all gone. RULING 3: the eight-module compliance "
            "panel is SPLIT into two panels, one per current category. RULING 4: every en dash "
            "and em dash in user-facing text is replaced by words, with the vendored library "
            "stopped and named. RULING 5: seven mis-filed panels are corrected, including the "
            "two alias cases. RULING 6: the deep-dive grouping loop's bound is DERIVED from the "
            "taxonomy, so the eleventh project-level category renders. Every count of modules or "
            "categories on a served page now DERIVES from registry_index() and service_index(); "
            "the handbook's '96 registered modules' is gone. NOTHING IS COMPUTED DIFFERENTLY AND "
            "NO STORED FIGURE MOVED: no formula, band, threshold, calibration, abstention rule "
            "or population moved, voting is still exactly A1.7 and A1.8, 63 modules are in "
            "service of 101 registered, and the behaviour digest is reproduced identically. ALL "
            "SIX SEQUENCE-BEARING FILES MOVED, each on its own ruling's authority and each "
            "carrying its own named exception record in participant_packages.py and in the v19 "
            "checksum record's header. The v33 candidate, its identity, its gate and its release "
            "records are preserved unchanged as the historical evidence for anything computed "
            "under v33."),
        "simulation_version": "sim-2026.08-v34",
        "participant_package": "og-participant-2026.08-v19",
    ...
    target = FREEZE / "run51_freeze_candidate_identity.json"
