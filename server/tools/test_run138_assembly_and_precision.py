...
class section:
    """One T of the order, isolated.

    RUN 135C, RULING R4: a suite that CRASHES prints no RESULT line and is indistinguishable,
    to anyone reading output, from one that had nothing to say. An exception raised inside a
    section is therefore recorded as a FAILED CHECK for that section and the remaining sections
    still run. Nothing is suppressed silently: the exception is printed.
    """
    ...
    def __exit__(self, exc_type, exc, tb):
        ...
        if exc_type is not None:
            fail += 1
            print(f"  FAIL  {self.title.split('.')[0]} raised {exc_type.__name__}: {exc}")
        return True


def src_of(si: dict, field: str) -> dict:
    """The provenance record for `field`, or an empty one.

    Never a KeyError: under R4 a suite that crashes prints no RESULT line and is
    indistinguishable from one that had nothing to say. A missing provenance record is a
    FAILED CHECK here, not a dead run -- which is what makes the fault injections legible.
    """
    return (si.get("sources") or {}).get(field) or {}
...
# =========================================================================== T1
with section("T1. Both a pay application and a monthly report present."):
    ...
    check("T1 ac provenance docType", src_of(both, "ac").get("docType"), "monthly_report")
...
# =========================================================================== T2
with section("T2. Pay application only -- ac absent, and every ac-dependent module abstains."):
...
# =========================================================================== T3
with section("T3. Monthly report only -- ac present, EVM eligible where the other inputs exist."):
    mr_only = assemble_signal_inputs([MR])
    check("T3 ac", mr_only["ac"], AC_STATED)
    check("T3 ac provenance docType", src_of(mr_only, "ac").get("docType"), "monthly_report")
...
# =========================================================================== T4
with section("T4. The retainage conflict -- identical BAC/EV/PV/period, two accounts of cost."):
...
# =========================================================================== T5
with section("T5. actualPctComplete selection is unchanged, and deliberately so."):
    ...
    check("T5 actualPctComplete comes from the pay application",
          src_of(both, "actualPctComplete").get("docType"), "pay_application")
...
# =========================================================================== T6
with section("T6. CPI precision -- a true index in [0.9995, 1.0) is stored unrounded, and "
             "bands from the unrounded value."):
...
# =========================================================================== T7
with section("T7. A1.8 edge -- CPI exactly 0.90 bands identically across a budget sweep."):
...
# =========================================================================== T8
with section("T8. No historical rewrite -- a recompute writes a NEW row and changes nothing."):
