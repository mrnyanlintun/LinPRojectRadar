#!/usr/bin/env python3
"""
RUN 43. THE SUCCESSOR FREEZE CANDIDATE IDENTITY.

Writes research/freeze/run43_freeze_candidate_identity.json in exactly the shape Run 41 wrote its
...
Usage: python tools/build_run43_candidate_identity.py [--candidate <commit>]
"""
...
PARENT_IDENTITY = FREEZE / "run42_freeze_candidate_identity.json"
...
def main() -> int:
    ...
    out: dict = {
        ...
        "supersession_reason": (
            "Run 43 retires 38 of the 101 registered modules FROM SERVICE, on the owner's ruling "
            "of 2026-08-21. Retirement is a statement about the taxonomy and the explanation "
            "burden, not a claim that any arithmetic is wrong: every retired module keeps its "
            "registry entry, its formula function and its audit lineage, and run_module() on all "
            "101 identifiers returns output byte-identical to v27. What changes is which modules "
            "the production paths enumerate and which reach a participant surface, and that is a "
            "change of executable behaviour - the client taxonomy, the ledger population, the "
            "training abstention list and the Portfolio Health snapshot all move - so the freeze "
            "is superseded rather than amended. The v27 candidate, its identity, its gate and "
            "its release records are preserved unchanged as the historical evidence for anything "
            "computed under v27."),
        "simulation_version": "sim-2026.08-v28",
        "participant_package": "og-participant-2026.08-v14",
    ...
    # THE RETIREMENT AUTHORITY. Named as its own group because it is the whole of the Run 43
    ...
    target = FREEZE / "run43_freeze_candidate_identity.json"
