#!/usr/bin/env python3
"""
RUN 65. CATEGORIES CARRYING A STATUS, ON A REAL PROJECT, COUNTED BEFORE AND AFTER.
...
LABEL = sys.argv[1] if len(sys.argv) > 1 else "run65"
OUT = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path("run65_capture.json")
...
def doc_bytes(tag): return f"%PDF-1.4 RUN65 {tag}\n".encode()
...
with Session() as s:
    ...
    if row is None:
        s.add(Participant(pseudonymous_code="R65-ADMIN", role="ResearchAdmin",
                          access_token_hash=hash_access_token(ADMIN)))
    else:
        row.access_token_hash = hash_access_token(ADMIN)
    if s.scalar(select(Project).where(Project.legacy_id == D)) is None:
        s.add(Project(legacy_id=D, doc={"id": D, "name": "Run 65 voting fixture",
                                        "signals": {}, "events": []}))
    s.commit()

admin = post({"action": "researchlogin", "access_token": ADMIN})["session_token"]
created = post({"action": "adminparticipantcreate", "session_token": admin,
                "pseudonymous_code": f"R65-PM-{int(time.time())}", "role": "Participant",
...
if os.environ.get("RUN65_FIXTURE_ONLY"):
