...
LABEL = sys.argv[1] if len(sys.argv) > 1 else "run66"
OUT = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path("run66_capture.json")
...
with Session() as s:
    ...
    if row is None:
        s.add(Participant(pseudonymous_code="R67-ADMIN", role="ResearchAdmin",
                          access_token_hash=hash_access_token(ADMIN)))
    else:
        row.access_token_hash = hash_access_token(ADMIN)
    if s.scalar(select(Project).where(Project.legacy_id == D)) is None:
        s.add(Project(legacy_id=D, doc={"id": D, "name": "Run 67 measurement fixture",
                                        "signals": {}, "events": []}))
    s.commit()

admin = post({"action": "researchlogin", "access_token": ADMIN})["session_token"]
created = post({"action": "adminparticipantcreate", "session_token": admin,
                "pseudonymous_code": f"R67-PM-{int(time.time())}", "role": "Participant",
...
if os.environ.get("RUN67_FIXTURE_ONLY"):
