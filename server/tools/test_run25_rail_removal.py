...
# subject is A1.1 -- 1 module id removed from the registry at Run 96 or Run 97 and no module
...
print("RETIRED: test_run25_rail_removal.py measures A1.1, removed at Run 96/97 (88e6ca0); excluded from qualification coverage "
...
check(pt.PINNED.name in ("run25_production_tree.sha256", "run26_production_tree.sha256",
                        ...
                        # RUN 33 extends the chain once more, for the same reason and by the same
                        # rule: the Portfolio Health canonical remediation ADDED canonical_v8.py
                        # and portfolio_health.py and CHANGED documents.py, models.py,
                        # project_data.py and registry.py, so the pin moves to the Run-33
                        ...
                        # RUN 36 extends the chain once more, by the same rule: the A1.1 closure
                        # CHANGED models_sim.py -- withdrawing a status band drawn over an
                        # UNSUPPORTED parameter -- and created no production file, so the pin
                        # moves to the Run-36 manifest and the Run-35 closure manifest is kept
                        # addressable as its parent.
                        "run36_production_tree.sha256",
                        # RUN 41 extends the chain once more, by the same rule. The owner ruled
                        # that both HIGH defects Run 40 confirmed be fixed before participant
                        ...
                        # RUN 43 extends the chain once more, by the same rule. The owner ruled
                        # that 38 of the 101 registered modules be retired FROM SERVICE; that
                        # CHANGED eleven production files and ADDED none, so the pin moves to
                        # the Run-43 manifest and the Run-42 manifest is kept addressable as its
                        # parent.
                        "run43_production_tree.sha256",
                        # RUN 44 extends the chain once more, by the same rule. The owner ordered
                        # the four participant-facing render defects Run 43J diagnosed repaired;
                        # that CHANGED six production files and ADDED none, so the pin moves to
                        # the Run-44 manifest and the Run-43 manifest is kept addressable as its
                        # parent.
                        "run44_production_tree.sha256",
                        # RUN 45 extends the chain once more, by the same rule. The owner signed
                        # off the canonical field classification and retrieval now resolves by
                        ...
                        "run48_production_tree.sha256", "run49_production_tree.sha256",
                        # RUN 51 extends the chain once more, by the same rule. The delivery of
                        ...
                        # RUNS 54 AND 55 extend the chain once more, and this is the FIRST
                        # extension in which production ROOT ENTRIES DISAPPEAR rather than merely
                        # change: assets/js/deepdive.js and research/deepdive.html are REMOVED
                        # and seven files CHANGED, so the pin moves to the Run-55 manifest and
                        # the Run-52 manifest is kept addressable as its parent. The rule this
                        ...
                        # RUN 57 extends the chain once more, by the same rule: EXACTLY FOUR
                        # production files changed (assets/css/radar.css, assets/js/detail.js,
                        # assets/js/ingest.js and server/app/simulation/models.py) and NOTHING
                        # was added, removed or renamed, so the pin moves to the Run-57
                        # manifest and the Run-56 manifest is kept addressable as its parent.
                        # The rule this tuple encodes -- that the pin may only ever move
                        # FORWARD along a chain of manifests each of which names its parent --
                        # is unchanged, and the "a root that has vanished is a freeze failure"
                        # guard is NOT softened. NOTHING IS REMOVED FROM THIS TUPLE.
                        "run57_production_tree.sha256",
                        # RUN 59. Six production files changed (assets/js/decision-ui.js,
                        # p0-baseline/MODULE_TAXONOMY.md and four comments in server/app) and
                        # NOTHING was added, removed or renamed, so the pin moves to the Run-59
                        # manifest and the Run-57 manifest is kept addressable as its parent.
                        # NOTHING IS REMOVED FROM THIS TUPLE.
                        "run59_production_tree.sha256",
                        # RUN 62. Three production files changed (assets/js/detail.js,
                        # assets/js/taxonomy.js and assets/js/workspace.js) and NOTHING was
