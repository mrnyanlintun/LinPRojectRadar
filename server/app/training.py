...
def _state_view(session: Session, run: TrainingRun, project: Project) -> dict[str, Any]:
    ...
    return {
        ...
        # Training upgrade run 2: the trade shortage, and what the crews are earning.
        "resource_notice": resource_position(run.state),
...
def a_trainingdecision(session: Session, payload: dict, secret: str, ttl: int) -> dict[str, Any]:
    ...
    valid = DECISIONS + RESPONSES + QUALITY_DECISIONS + RESOURCE_DECISIONS
    if decision not in valid:
        return err(f"decision must be one of: {', '.join(valid)}")
