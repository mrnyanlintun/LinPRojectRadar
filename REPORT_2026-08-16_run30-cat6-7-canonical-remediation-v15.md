...
**Real-corpus Category-7 results generated by legacy proxy = 0.**

> **CORRECTION, made in the Run-30 FINAL closure.** This paragraph originally read "Eighteen
> populated rows became abstentions." **That was a counting error.** The measured figure is
> **sixteen**. Seventeen legacy runners executed, but B2.19 ran its runner and abstained under
> v15 as well, so the count of runners and the count of populated analytical rows are different
> quantities and eighteen matched neither. The full accounting is in the final-closure addendum
> below and in `code_audit/run30_cat7_before_after_ledger_reconciliation.csv`.

**The ledger is emptier and that is the correct outcome**; not one old reading was preserved to
keep a row occupied.
...
# RUN 30 FINAL CLOSURE ADDENDUM — lineage-state semantics and the ledger-count reconciliation

Two issues remained after the operational repointing, and both were mine.

## A. Category-7 lineage semantics

### Why the eleven declarations were removed

Every one of them said the same thing: that the module's reading rested on the two performance
indices and the document risk score, through the earned-value body and the document body. That was
**true of the implementations they described** — and it is exactly why those implementations were
proxies. After the repointing it was true of nothing: the canonical routes read no index and no
document risk score. Leaving them would have asserted a dependence that had stopped existing, and
a false `CORRELATED` edge into the earned-value body is the dangerous direction, because a
consumer uses it to **suppress** corroboration that is really there.

### Why they were NOT replaced with independent bodies

What a governed epistemic structure rests on is whatever its assessor read, and **this platform
does not know that**. Minting a body per module id would have manufactured precisely the
independence the lineage table exists to stop being assumed — twenty transformations of unknown
provenance becoming twenty corroborating sources. The owner's condition was not that they be
restored, but that the resulting state be *explicit*. It was not: a row with no declaration was
mechanically indistinguishable from one whose independence had been established, because both
carried nothing.

### Which vocabulary was used, and why

**The repository's own, extended rather than replaced.** `lineage.py` already declared nine
EVIDENCE RELATIONSHIPS (INDEPENDENT, SAME_SOURCE, DERIVED, SAME_SOURCE_TRANSFORM, CORRELATED,
SYNTHESIZED, QUALITY_METADATA, GOVERNANCE_OUTPUT, DECISION_OUTPUT). I did **not** add a tenth,
because all nine presuppose that a body has been *established* — they answer "what is this
signal's relation to its body", and none can say "no body has been established".

That state already existed in behaviour: `fusion.py` implements it as its **FUSION.1** decision of
Run 20 cycle 9 — an undeclared signal goes into one shared UNRESOLVED body, is not combined as
independent, and is reported through `unresolved_module_ids`. So the four states **name what
fusion already does**; they introduce no semantic:

| state | meaning |
|---|---|
| `LINEAGE_ESTABLISHED_INDEPENDENT` | a body is established and rests on its own. The only state that may corroborate. |
| `LINEAGE_ESTABLISHED_DEPENDENT` | a body is established and is shared with, or derived from, another. |
| `LINEAGE_UNRESOLVED` | **no body established. Not independence, and not a defect in the row** — the truthful state of a structure whose assessors' own sources are unknown here. |
| `LINEAGE_NOT_APPLICABLE` | the module produces no reading at all, so there is no signal for a lineage statement to be about. Distinct from UNRESOLVED. |

`lineage_status()` **derives** the state from the shipped declaration table. There is no branch
anywhere that can put `ESTABLISHED_INDEPENDENT` on a row whose declaration does not say so.

### Source provenance kept separate from independence

A row may know exactly who assessed its structure, from what, in which period — and still report
`LINEAGE_UNRESOLVED`, because **knowing the source of a structure is not knowing what its assessor
read**. Both are reported side by side; neither is inferred from the other. Measured on B2.14 with
a governed structure supplied: `source_provenance` is populated, `lineage_status` is UNRESOLVED,
`independence_established` is false, and the row carries a sentence saying so — while the
analytical reading itself is produced normally. Unresolved lineage does not suppress the reading;
it refuses only corroboration through it.

## B. The 20-row Run-31 lineage handoff

`code_audit/run30_cat7_lineage_handoff.csv`. **20 rows, 20 unique modules, 0 ambiguous blank
states, 0 fabricated independent bodies, 0 rows claiming independence.**

| module | operational state | source known | lineage status | evidence body | independence | Run-31 action |
|---|---|---|---|---|---|---|
| B2.1 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.2 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.3 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.4 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.5 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.6 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.7 | DISABLED_UNSAFE | no | `LINEAGE_NOT_APPLICABLE` | none established | no | E |
| B2.8 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.9 | DISABLED_UNSAFE | no | `LINEAGE_NOT_APPLICABLE` | none established | no | E |
| B2.10 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.11 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.12 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.13 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.14 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.15 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.16 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.17 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.18 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.19 | ADVISORY_ONLY | no | `LINEAGE_UNRESOLVED` | none established | no | D + C |
| B2.20 | DISABLED_UNSAFE | no | `LINEAGE_NOT_APPLICABLE` | none established | no | E |

By the handoff contract's five categories: **A (established and eligible) = 0; B (established but
dependent) = 0; C (lineage unresolved) = 17; D (no evidence, module abstained) = 17; E
(disabled/archive) = 3.**

C and D are **orthogonal and on this corpus both apply to the same seventeen**, so both are
recorded rather than one being chosen. Whether a module produced evidence is a fact about *this
period*; whether its lineage is established is a fact about the structure's *provenance*. Forcing
one letter would have made Run 31 guess which question had been answered — and supplying the
missing structures would move those rows out of D while leaving them in C.

"source known = no" on every row is because this corpus supplies no Category-7 structure at all.
Where one *is* supplied the provenance is recorded, as B2.14 above demonstrates.

## C. The 17-versus-18 reconciliation

`code_audit/run30_cat7_before_after_ledger_reconciliation.csv`, 20 rows. Both analytical lines
executed through `registry.run_all` on the controlled real-corpus signal inputs. Every count is a
measurement.

| module | v15 runner ran | v15 result | v15 row | v16 disposition | disabled |
|---|---|---|---|---|---|
| B2.1 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.2 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.3 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.4 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.5 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.6 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.7 | no | (short-circuited) | abstained | DISABLED | **yes** |
| B2.8 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.9 | no | (short-circuited) | abstained | ARCHIVED | **yes** |
| B2.10 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.11 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.12 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.13 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.14 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.15 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.16 | yes | **Red** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.17 | yes | **Amber** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.18 | yes | **Yellow** | computed | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.19 | **yes** | **(abstained)** | **abstained** | NOT_ESTIMABLE_STRUCTURE_ABSENT | no |
| B2.20 | no | (short-circuited) | abstained | DISABLED | **yes** |

### The seven counts, measured

| quantity | value |
|---|---|
| v15 proxy runners executed | **17** |
| v15 populated analytical ledger rows | **16** |
| v15 disabled/archive rows | **3** |
| v16 canonical operational runners | **17** |
| v16 canonical numerical results | **0** |
| v16 canonical abstentions | **17** |
| v16 disabled/archive rows | **3** |

**Populated analytical rows that became abstentions = 16.**

### Why the earlier report said eighteen

**It was a counting error in prose, not an extra live row.** Seventeen runners executed, but only
sixteen produced a populated analytical reading, because **B2.19 CRITIC-TOPSIS ran its legacy
runner and abstained under v15 too** — it already required an explicit decision structure this
corpus does not carry. The count of *runners* and the count of *populated rows* are therefore
different quantities, and eighteen matched neither. No Category-7 analytical row existed outside
the seventeen runners; every one of the twenty is accounted for by a production runner or by the
disabled gate. The Run-30 report has been corrected in place at the paragraph that carried the
error, with the correction marked rather than silently rewritten.

### Disabled/archive ledger semantics, stated separately

B2.7 Plithogenic, B2.9 Quantum and B2.20 Hypersoft carry ledger rows expressing their disabled or
archived state **under both lines**. They are counted in **neither** the before nor the after
analytical totals. Part of the confusion was exactly this boundary, so it is stated on every row
of the reconciliation rather than left implicit in a total: `disabled_or_archive = yes`,
`v15_ledger_row_existed = yes`, and never a populated analytical reading.

## D. The four lineage faults

`test_run30_lineage_semantics.py` (39/39), record in `code_audit/run30_lineage_fault_injection.csv`.
**4 attempted, 4 `RED_THEN_GREEN`, 0 `INJECTION_NOT_APPLIED`.** Each injection was confirmed
applied by re-reading the attribute off the module; baseline rechecked after the campaign.

| fault | injected | result |
|---|---|---|
| **A** — an UNRESOLVED state declared ESTABLISHED_INDEPENDENT without evidence | `models_cat7.lineage_status` returns INDEPENDENT unconditionally | RED → GREEN |
| **B** — lineage status removed from the row entirely | `models_cat7._lineage_block` returns a block with no status | RED → GREEN |
| **C** — two transformations of one body given different invented body ids | `lineage.evidence_body_of` returns `BODY_OF_<module>` | RED → GREEN |
| **D** — UNRESOLVED treated as eligible independent evidence in fusion | `fusion.fuse_signals` gives each undeclared signal its own body | RED → GREEN |

**Fault C's first formulation was wrong and is recorded rather than quietly replaced.** It asked
whether two modules sharing the earned-value body produce the *same* body string. They do not:
A1.1 also belongs to the document body and A1.2 to the reporting history, so their joined
identifiers differ legitimately. Belonging to more than one body is not an invented body. The
property that actually matters — and is now asserted — is that the identifier comes from the
**declared groups** and never from the module id, so two modules sharing a group still share it.

## E. The version decision, from behaviour

**`sim-2026.08-v16` STANDS. It is not overwritten and no stamp was added.**

Decided by execution, not file type (`test_run30_final_version_decision.py`, 19/19). The v16
package was extracted from git object `b7709cf`, imported, and run beside the current one through
`registry.run_all` and `compute.compute_project` on identical input:

- computed set **identical**; abstaining set **identical** — no eligibility outcome moved;
- every row **identical apart from the `lineage` metadata key**;
- `project_status` Amber → Amber, `categories_voting` 1 → 1, `voting_module_ids` unchanged,
  `project_conflict_state` unchanged, category rollup unchanged;
- the only difference anywhere: the `lineage` key on the twenty Category-7 rows.

**The one way that answer could be wrong** is if the new field fed eligibility. Fault D proves
lineage status **is** behavioural in fusion — treating UNRESOLVED as independent sharpens Amber
belief from **0.7000 past 0.9**. So the separate fact is proved too: `compute` builds every fusion
input from `lineage_for(module_id)`, the declaration table, and **never reads the lineage key off a
module's result row**; rewriting that key on a returned row changes no fused answer. The behaviour
Fault D demonstrates is FUSION.1's, shipped since Run 20 cycle 9 and unchanged by this closure.
What this closure added is the *name*.

## F. Operational re-verification, after the metadata change

`test_run30_cat7_operational_route.py` re-run through `registry.run_module` with the interpreter
profiled — **not** by calling `canonical_v5`. 67/67: 20 identities accounted for, canonical route
17/17 operational, **legacy proxy routes 0**, disabled/archive still non-operational, 0 fabricated
independent lineage, 0 ambiguous lineage.

## G. Packages and the 24-row closure

- **Participant: v5 preserved, no successor.** No participant byte moved — the served
  defensibility evidence object is generated from the registry and the structure maps, and this
  closure touched neither. Regenerating it produced byte-identical output. v4 stays pinned to
  `ce03eb1`; exactly one record matches the live tree and it is the declared current link.
- **Synthetic: no successor.** No package byte moved. All predecessors preserved.
- **Freeze:** `RUN30_FINAL_CLOSURE_FREEZE_2026-08-16.json`, identifier `...-RUN30-FINAL-CLOSURE-V16-2`,
  superseding `...-RUN30-CLOSURE-V16-1`. Parent untouched. No new declared-changes manifest:
  `lineage.py` is declared by Run 29 and `models_cat7.py` by Run 30's own new-file list.
- **Closure table regenerated:** 24 rows, 24 unique identities, Category-7 production canonical
  **20/20**, legacy reachable **0**, ambiguous Category-7 lineage **0**, fabricated independent
  bodies **0**, ledger-count discrepancy **0**, unaccounted **0**. Two columns added:
  `lineage_status` and `independence_established`.

## H. What this final closure did not do

No Category-9 closure (Run 31 — and every row still carries `signal_qualification = unqualified`);
no calibration (Run 33); no Category-10 placement (Run 32); no lineage invented; nothing
activated; voting still exactly A1.7 and A1.8; A3.4 still disabled; participant sequence unchanged;
Run 31 not launched.
