...
const DS_DEFENSIBILITY = {
 ...
 "intro": "This handbook explains how every analytical capability on the platform is designed, and what uncertainty model it rests on. It also states, per capability, what evidence the repository actually holds for it. It is organized by data science method, not by module number: readers meet the probabilistic reasoning first. Three signal types, EVM forecasting, statistical anomaly detection, and document-signal extraction, are fused into governed status recommendations through evidence combination, with a human judgment layer recording every concurrence or override. The research artifact is evaluated with public-program controls professionals. That evaluation is of the instrument, not of any module's predictive performance.",
 "oneSentence": "This does not dress business rules as statistics; it separates estimation, calibrated measurement, and evidence combination under epistemic uncertainty, and states for each what evidence exists and what would be required to validate it.",
 "executiveFindings": [
  "The module registry ({{registered}} registered computations, of which {{supplied}} is a value the extraction model supplies rather than the analytical server) is present and code-aligned. Module names, method classes, required inputs, computation source, and current implementation status are no longer unspecified.",
  ...
  "axisA": {
   ...
   "tiers": [
    [
     ...
     "Convergence, residual/holdout diagnostics, false-alarm/detection design, reference-implementation comparison, simulation verification and validation",
   ...
   "modules": [
    {
     ...
     "implementationFidelity": "The sampling engine is genuine seeded Beta-PERT Monte Carlo. The risk-spread mapping and optimistic/most-likely/pessimistic bounds are designed heuristics, so the result is reproducible demonstration evidence. It is not a calibrated forecast, and no calibration set for it exists in this repository.",
    ...
    },
    {
     ...
     "implementationFidelity": "The code uses a precision-weighted normal-normal update, but the prior and likelihood variances are designed rather than estimated. It is a Bayesian-form demonstration, not an fitted to observed outcomes, which has not been done here Bayesian forecast.",
     "accreditationBasis": "Bayesian EAC methods are established in EVM literature (e.g., Kim and Reinschmidt). Validation for this method would consist of comparing posterior forecasts against realized finals on seeded multi-period histories. None of that has been performed on this platform. What the repository holds is known-answer testing of the stated formula and, for many modules, a domain and boundary enumeration. That supports the claim that the arithmetic is independently verified for the stated formula. It does not establish empirical validation or calibration.",
     "validationRequired": "Verify equations and code; test deterministic seeds; perform convergence or residual diagnostics; use seeded known-pattern cases; conduct sensitivity analysis; compare with a trusted reference implementation; document calibration and holdout performance where data permit.",
     "standardsAlignment": "NIST AI RMF 1.0: MAP and MEASURE, with MANAGE for response; NIST TEVV for validity, reliability, uncertainty, benchmark, robustness, and monitoring evidence; ISO/IEC 42001: clauses 6 and 8-10; ISO/IEC 25059 quality characteristics.",
    ...
    },
    {
     ...
     "engineeringProblem": "TCPI = (BAC-EV)/(BAC-AC); bands at 1.00 and 1.10. Measures required future efficiency.",
     "detProbSplit": "Deterministic input checks and arithmetic/rule computation followed by a deterministic band or decision rule. Uncertainty resides in measurement error, data completeness, threshold calibration, and source reliability rather than random sampling.",
     ...
     "accreditationBasis": "The formula is the Project Management Institute one (A Guide to the Project Management Body of Knowledge, 6th edition, 2017, section 7.4.2.2; Practice Standard for Earned Value Management, 2nd edition, 2011). The first boundary, 1.00, is the one that source defines: at or below it the remaining budget suffices at the efficiency already planned. The second, 1.10, applies the 0.10 cumulative cost index stability finding of Christensen and Heise (National Contract Management Journal 25(1), 1993, pages 7 to 15) by an inference that is stated rather than implied. THE LIMIT OF THAT, and it is the whole limit: the boundaries come from a published source, and how often the reading is right has never been measured. There is no labelled holdout corpus and no expert reference standard for this platform, so false-positive and false-negative performance is unknown.",
     "validationRequired": "Verify formula and threshold boundaries; trace each input to provenance; run missing/stale/contradictory-data tests; measure false-positive and false-negative performance on labelled cases once a labelled corpus exists; confirm that the output abstains when required inputs are absent.",
     "standardsAlignment": "NIST AI RMF 1.0: MAP, MEASURE, and MANAGE; ISO/IEC 42001: clauses 7-10; ISO/IEC 23894 risk analysis/evaluation; W3C PROV-O for traceability where source records drive the indicator.",
     "permittedProhibitedClaims": "Allowed: \"The platform implements the documented TCPI formula and bands it on boundaries traced to published sources.\" Prohibited: claims of predictive accuracy, causal validity, measured error rates, or production certification without field evidence.",
     "governanceRole": "Enters the signal package with provenance, method version, input completeness, uncertainty/confidence, status, evidence metric, and abstention state; cannot authorize action without human judgment.",
     "defenseQuestions": [
      {
       "question": "Is a TCPI of 1.074 'statistically' Yellow?",
       "answer": "No test is claimed and no calibration is claimed. The boundaries are 1.00, which the Project Management Institute defines as the point at which the remaining budget stops being sufficient at the efficiency already planned, and 1.10, which applies the 0.10 cumulative index stability finding of Christensen and Heise (1993) by a stated inference. What the harness proves is that the code honours those boundaries. How often a project on one side of a boundary turns out worse than one on the other has not been measured here."
      },
      {
    ...
    },
    {
     ...
     "assuranceClass": "A",
     "assuranceDescription": "Formula/rule-faithful implementation that is directly testable, subject to input and threshold validity",
     "engineeringProblem": "VAC = BAC - EAC, banded relative to BAC at zero per cent and at minus 11.11 per cent.",
     "detProbSplit": "Deterministic input checks and arithmetic/rule computation followed by a deterministic band or decision rule. Uncertainty resides in measurement error, data completeness, threshold calibration, and source reliability rather than random sampling.",
     "uncertaintyMethod": "A ratio computed exactly from reported figures (no sampling error in the arithmetic). Uncertainty enters through the inputs (measurement/reporting error in EV, AC, PV) and is handled at the governance layer: 4-tier bands act as control limits - decision thresholds calibrated to industry evidence, not p-values.",
     "explainability": "Formula/rule trace: named inputs with provenance, exact transformation, intermediate values, threshold bands, status result, and abstention reason when inputs are missing.",
     "oversightLevel": "FLAG",
     ...
     "accreditationBasis": "The measure is the Project Management Institute one (A Guide to the Project Management Body of Knowledge, 6th edition, 2017, section 7.4.2.2), which defines a negative variance at completion as a forecast overrun, so zero is a boundary the source specifies. Because the forecast is the index-based one, the percentage is an exact restatement of the cost performance index, and the second boundary is an index of 0.90, applying the 0.10 stability finding of Christensen and Heise (National Contract Management Journal 25(1), 1993, pages 7 to 15) by a stated inference; that finding is conditional on the project being past twenty per cent complete and this measure does not read percent complete. THE LIMIT: the boundaries are sourced, and false-positive and false-negative performance has never been measured, because no labelled holdout corpus and no expert reference standard exist for this platform.",
     "validationRequired": "Verify formula and threshold boundaries; trace each input to provenance; run missing/stale/contradictory-data tests; measure false-positive and false-negative performance on labelled cases once a labelled corpus exists; confirm that the output abstains when required inputs are absent.",
     "standardsAlignment": "NIST AI RMF 1.0: MAP, MEASURE, and MANAGE; ISO/IEC 42001: clauses 7-10; ISO/IEC 23894 risk analysis/evaluation; W3C PROV-O for traceability where source records drive the indicator.",
   ...
   "modules": [
    {
     ...
     "accreditationBasis": "PERT/CPM and SRA are standard (PMI SP for Scheduling; GAO Schedule Assessment Guide). Band edges are exercised in the band harness, which checks that each boundary is reached and that neither side of it is entered by accident; that is a check of the implementation, not of the boundary's fitness. Validation of the schedule risk analysis would consist of convergence checks.",
     "validationRequired": "Verify formula and threshold boundaries; trace each input to provenance; run missing/stale/contradictory-data tests; calibrate bands on synthetic scenarios and, later, non-confidential field cases; confirm that the output abstains when required inputs are absent.",
     "standardsAlignment": "NIST AI RMF 1.0: MAP, MEASURE, and MANAGE; ISO/IEC 42001: clauses 7-10; ISO/IEC 23894 risk analysis/evaluation; W3C PROV-O for traceability where source records drive the indicator.",
  ...
  },
  {
   ...
   "modules": [
    ...
    },
    {
     ...
     "accreditationBasis": "Poisson/queueing theory is canonical (M/M/1 assumptions stated); validation of the agent-based and discrete-event methods would consist of face validation and extreme-condition tests (Sargent's simulation verification and validation framework - the citable accreditation standard for simulation models).",
     "validationRequired": "Verify formula and threshold boundaries; trace each input to provenance; run missing/stale/contradictory-data tests; calibrate bands on synthetic scenarios and, later, non-confidential field cases; confirm that the output abstains when required inputs are absent.",
     "standardsAlignment": "NIST AI RMF 1.0: MAP, MEASURE, and MANAGE; ISO/IEC 42001: clauses 7-10; ISO/IEC 23894 risk analysis/evaluation; W3C PROV-O for traceability where source records drive the indicator.",
   ...
   "modules": [
    ...
    },
    {
     ...
     "defenseQuestions": [
      {
       "question": "Validation?",
       "answer": "Sargent verification and validation: extreme-condition tests (zero rework -> plan trajectory; high rework -> known death-spiral shape from SD literature, Cooper's rework cycle)."
      }
     ],
     "requiredInputs": [
      "cpi",
   ...
   "modules": [
    {
     ...
     "implementationFidelity": "Code-aligned formula/rule review required; defend only at the documented assurance class.",
     "accreditationBasis": "Shafer (1976) primary source. Validation for this method would consist of the fusion-table unit tests and by the calibration probe protocol (known-band document sets must reproduce target category status). None of that has been performed on this platform. What the repository holds is known-answer testing of the stated formula and, for many modules, a domain and boundary enumeration. That supports the claim that the arithmetic is independently verified for the stated formula. It does not establish empirical validation or calibration.",
  ...
  },
  {
   ...
   "modules": [
    ...
    },
    {
     ...
     "assuranceClass": "C",
     "assuranceDescription": "Refactor or rename before making a formal method claim",
     "engineeringProblem": "SOBER GROUNDING REQUIRED (open item): defensible only as non-commutative probability for order-effects in judgment (Busemeyer and Bruza literature) - NOT physics claims. If it cannot be grounded that way, cut it before the defense.",
     "detProbSplit": "Deterministic algebra over epistemic representations (belief masses, memberships, linguistic terms, rankings, or votes). No repeated-sampling probability is implied unless explicitly stated.",
     "uncertaintyMethod": "Represents epistemic uncertainty (vague, conflicting, or partially trusted evidence) that classical probability handles poorly: membership degrees, belief/plausibility intervals, or hesitancy terms instead of a single probability mass. Output is a graded status assessment carrying its own confidence.",
     "explainability": "Evidence trace: input statuses, memberships or masses, reliability discounts, conflict/indeterminacy, combination steps, final score, and parameter-sensitivity table. SHAP/LIME is not appropriate for these symbolic formalisms.",
     "oversightLevel": "INFORM_ONLY_FLAG",
   ...
   "modules": [
    {
     ...
     "implementationFidelity": "Code-aligned formula/rule review required; defend only at the documented assurance class.",
     "accreditationBasis": "Grounded in data-quality dimensions literature (accuracy/completeness/timeliness/consistency - Wang and Strong 1996). Validation for this method would consist of injecting known gaps/contradictions into synthetic sets. None of that has been performed on this platform. What the repository holds is known-answer testing of the stated formula and, for many modules, a domain and boundary enumeration. That supports the claim that the arithmetic is independently verified for the stated formula. It does not establish empirical validation or calibration.",
     "validationRequired": "Verify formula and threshold boundaries; trace each input to provenance; run missing/stale/contradictory-data tests; calibrate bands on synthetic scenarios and, later, non-confidential field cases; confirm that the output abstains when required inputs are absent.",
     "standardsAlignment": "NIST AI RMF 1.0: GOVERN, MAP, and MEASURE; ISO/IEC 42001: clauses 7-10; ISO/IEC 23894: risk-information quality, monitoring, and review; W3C PROV-O for entity/activity/agent provenance; dataset documentation principles from datasheets.",
