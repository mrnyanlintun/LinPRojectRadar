...
Recommendation disclosed → Recorded decision, with rationale</pre>
    ...
    },
    {
      ...
      build: () => `
        ...
        <p>Conservative Dominance is a module, not the platform's status rule: it is the governance baseline reading over the four assembled signals, in which the worst credible signal drives the state, with explicit conflict typing. The platform's own posture rules are separate and are set out under Category status: the four performance categories average their banded modules&rsquo; scores and Delivery Quality takes the worst. The evidence-combination methods each provide an independent lens on the same evidence (probabilistic, set-theoretic, fuzzy, quantum). When the baseline and any of those lenses disagree, the disagreement itself is what gets recorded: the governance layer owns the residual uncertainty rather than picking a method. The governance layer consumes both the baseline and those cross-checks to produce the recommendation that gets recorded.</p>
      `,
    },
    {
      id: "module10",
...
/* ============================================================
  ...
  const CAT1_MODULES = [
    { n: "A1.1", name: "Monte Carlo EAC Forecast", mc: "Monte_Carlo",
      ...
      sources: "Pay Application (G702), Schedule of Values (G703), Cost Report, supplies BAC/EV/AC/CPI/SPI that seed the distribution. A positive budget and positive indices are required and none of them is ever substituted: the hundred-unit placeholder budget this once fell back to has been removed, and a budget or an index of zero now abstains.",
    ...
    { n: "A1.2", name: "CUSUM Anomaly Monitor", mc: "CUSUM",
    ...
    { n: "A1.5", name: "ARIMA CPI Forecast", mc: "ARIMA_Forecast",
    ...
    { n: "A1.10", name: "CPI Shrinkage Forecast", mc: "CPI_Shrinkage_Forecast",
    ...
    { n: "A1.11", name: "Independent EAC Reconciliation Index", mc: "Independent_EAC_Reconciliation",
  ...
  const CAT2_MODULES = [
    ...
    { n: "A2.2", name: "Line of Balance", mc: "Line_of_Balance_Velocity",
    ...
    { n: "A2.3", name: "CCPM Buffer Health", mc: "CCPM_Buffer_Health",
    ...
    { n: "A2.4", name: "Schedule Compression Index", mc: "Schedule_Compression",
    ...
    { n: "A2.5", name: "Float Consumption Rate", mc: "Float_Consumption",
      ...
      formula: "consumptionRate = consumedFloat / totalFloat; expectedConsumption = actualPctComplete/100; floatStress = consumptionRate / max(expectedConsumption, 0.01). Completion is required and is never assumed: the 50% default this once carried has been removed.",
      bands: [["green","Green","floatStress ≤ 1.0"], ["yellow","Yellow","1.0-1.3"], ["amber","Amber","1.3-1.6"], ["red","Red","&gt; 1.6"]],
      abstain: "totalFloat or consumedFloat missing, totalFloat ≤ 0, or no reported completion percentage. Float consumption is only meaningful against the work actually completed, so it abstains rather than assuming a halfway point.",
    ...
    { n: "A2.6", name: "S-Curve Deviation", mc: "SCurve_Deviation",
    ...
    { n: "A2.7", name: "Milestone Trend Analysis", mc: "Milestone_Trend",
    ...
    { n: "A2.8", name: "Look-Ahead Schedule Health", mc: "Lookahead_Health",
    ...
    { n: "A2.9", name: "Resource Loading Index", mc: "Resource_Loading",
  ...
  const CAT3_MODULES = [
    ...
    { n: "A3.3", name: "Labor Productivity Index", mc: "Labor_Productivity",
    ...
    { n: "A3.4", name: "Material Cost Variance", mc: "Material_Cost_Variance",
  ...
  const CAT4_MODULES = [
    ...
    { n: "A4.4", name: "NCR Rate", mc: "NCR_Rate",
      ...
      formula: "openRatio = ncrOpen / totalFindings, the open backlog against the audited nonconformance cohort. The backlog is carried across periods, so it is not measured against one period's intake.",
      bands: [["green","Green","open ratio &lt; 15%, or zero NCRs issued"], ["yellow","Yellow","15-30%"], ["amber","Amber","30-50%"], ["red","Red","≥ 50%"]],
      abstain: "ncrIssued, ncrClosed or ncrOpen missing, no audited cohort supplied, or a backlog larger than the cohort. Without a Quality Audit Report to define the cohort this computation abstains.",
    ...
    { n: "A4.5", name: "Weather Day Impact", mc: "Weather_Impact",
      ...
      formula: "weatherRatio = weatherDaysLost / floatRemaining (float from floatRemaining, or totalFloat − consumedFloat). Both the lost days and the float must be reported: the worst case this once asserted whenever no float figure existed has been removed.",
      bands: [["green","Green","zero weather days lost"], ["yellow","Yellow","weatherRatio ≤ 20% of float"], ["amber","Amber","20-50%"], ["red","Red","&gt; 50%"]],
      abstain: "weatherDaysLost missing or inferred rather than counted in a field report, a negative count, no float figure reported, or no positive float remaining. The float here is network derived and the document set does not yet carry an activity network, so this computation is expected to abstain until it does.",
      sources: "Daily / Weekly Field Report for the verified lost days, and a schedule update for the float. Nothing is estimated: an inferred figure abstains rather than computing with a note attached.",
    ...
    { n: "A4.6", name: "Change Order Frequency", mc: "CO_Frequency",
    ...
    { n: "A4.7", name: "Dispute Escalation Index", mc: "Dispute_Escalation",
    ...
    { n: "A4.9", name: "Procurement Lead Time Monitor", mc: "Procurement_Lead_Time",
      ...
      formula: "riskRatio = (longLeadDelayed + 0.5 × (longLeadAtRisk − longLeadDelayed)) / longLeadItemsTotal. A delayed item is an at-risk item that has already slipped, so it carries full weight and is counted once; the remaining at-risk items carry half. The result is a proportion of the long-lead set and cannot exceed 1.",
      bands: [["green","Green","&lt; 15%"], ["yellow","Yellow","15-30%"], ["amber","Amber","30-50%"], ["red","Red","≥ 50%"]],
      abstain: "longLeadItemsTotal, longLeadAtRisk or longLeadDelayed missing, an empty procurement log, a negative count, more items at risk than exist, or more delayed than at risk.",
    ...
    { n: "A4.10", name: "Specification Conflict Density", mc: "Spec_Conflict_Density",
  ...
  const CAT5_MODULES = [
    ...
    { n: "A5.3", name: "Tornado Risk Ranking", mc: "Tornado_Diagram",
  ...
  const CAT7_MODULES = [
    ...
    { n: "B2.16", name: "Spherical Fuzzy Sets", mc: "Spherical_Fuzzy",
  ...
  const CAT8_MODULES = [
    ...
    { n: "B3.2", name: "FAR/Agency EVMS Applicability Monitor", mc: "EVMS_Applicability",
    ...
    { n: "B3.3", name: "Versioned A-11 Capital Programming Conformance Check", mc: "A11_Conformance",
    ...
    { n: "B3.4", name: "EVMS Reporting Compliance Monitor", mc: "EVMS_Reporting_Compliance",
    ...
    { n: "B3.5", name: "Contract Modification Governance Check", mc: "Modification_Governance",
    ...
    { n: "A6.1", name: "Quality Compliance Index", mc: "Quality_Compliance",
      ...
      formula: "passRate = (itemsInspected − itemsFailed)/itemsInspected, from a real inspected and failed pair only; auditScore = qualityAuditScore if supplied, else passRate × 100. Neither the inspected count nor the failed count is ever substituted: a deficiency noted in a field report is a different quantity from an inspection lot that failed.",
      bands: [["green","Green","≥ 85/100"], ["yellow","Yellow","70-84"], ["amber","Amber","55-69"], ["red","Red","&lt; 55"]],
      abstain: "qualityDeficienciesNoted missing; no audited score and no inspected and failed pair; nothing recorded as inspected; or more items failed than inspected, which is an inconsistent pair rather than a project condition and would put the score outside the range a percentage can take.",
    ...
    { n: "A6.2", name: "Safety Performance Index", mc: "Safety_Performance",
    ...
    { n: "A6.3", name: "Environmental Compliance Rate", mc: "Environmental_Compliance",
      ...
      formula: "complianceRate = environmentalComplianceRate, the audited proportion of permit conditions met. There is no substitute for it: a count of how often the subject was raised in a meeting is not a measure of compliance, and the score once derived from that count has been removed.",
      bands: [["green","Green","≥ 95%"], ["yellow","Yellow","85-94%"], ["amber","Amber","70-84%"], ["red","Red","&lt; 70%"]],
      abstain: "environmentalIssuesDiscussed missing, no audited compliance rate supplied, or a rate outside nought to a hundred. Without an Environmental Compliance Report this computation abstains.",
    ...
    { n: "A6.4", name: "Contractor Performance Assessment Signal", mc: "Contractor_Performance",
      ...
      abstain: "overallRating, scheduleRating or costRating missing. A quality rating is used when present and is not required.",
  ...
  const CAT9_MODULES = [
    ...
    { n: "C1.3", name: "Source Reliability Weighting", mc: "Source_Reliability_Weighting",
    ...
    { n: "C1.4", name: "Audit Trail Completeness", mc: "Audit_Trail_Completeness",
    ...
    { n: "C1.7", name: "Reporting Frequency Index", mc: "Reporting_Frequency_Index",
  ...
  const CAT10_MODULES = [
    { n: "B4.1", name: "Multi-Objective Optimization", mc: "Multi_Objective_Optimization",
      purpose: "Reports which of an explicitly declared set of candidate alternatives are non-dominated across the declared objectives, so the trade-offs stay visible instead of being collapsed into a single score. It does not choose between them.",
      formula: "Dominance over the declared alternative set: alternative a dominates b when a is no worse than b on every objective and strictly better on at least one, with each objective's benefit/cost orientation read from the declared structure rather than assumed. Alternatives declared infeasible are excluded before dominance is computed. No single best alternative is selected, because choosing one from a non-dominated set requires preference information that is not governed here.",
      abstain: "the governed decision structure `decisionAlternatives` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `decisionAlternatives` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "A larger non-dominated set means the declared objectives genuinely conflict and the choice is a trade-off rather than an optimisation. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.2", name: "Linear Programming", mc: "Linear_Programming",
      purpose: "Solves a declared linear program exactly and reports the optimal vertex, the objective value attained there, and which constraints are binding at it.",
      formula: "Exact vertex enumeration over rationals. An optimum of a bounded feasible region lies at a vertex, and a vertex solves a square subsystem of the binding constraints, so every candidate is solved with exact fraction arithmetic and tested for feasibility against all declared constraints and bounds, including non-negativity. There is no tolerance to choose and no floating-point tie to resolve. An infeasible program is reported as infeasible rather than as a number.",
      abstain: "the governed decision structure `linearProgramModel` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `linearProgramModel` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "The binding constraints are the ones actually limiting the objective; relaxing a non-binding constraint changes nothing. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.3", name: "Constraint Satisfaction Analysis", mc: "Constraint_Satisfaction",
      purpose: "Classifies every complete assignment of the declared variables over their declared domains as feasible or infeasible against the declared constraint network, and reports which constraints each infeasible assignment violates.",
      formula: "A constraint satisfaction problem: variables, their domains, and constraints over them. Every complete assignment is evaluated against every declared constraint; no constraint may be skipped, and a constraint stated in a rule form this method does not evaluate is refused rather than treated as satisfied.",
      abstain: "the governed decision structure `constraintSatisfactionProblem` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `constraintSatisfactionProblem` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "An empty feasible set means the declared constraints cannot all be met at once, which is a statement about the declared problem and not a project condition. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.4", name: "What-If Scenario Matrix", mc: "WhatIf_Scenario_Matrix",
      purpose: "Compares declared candidate actions against declared scenarios cell by cell, so the outcome of each action under each future is visible side by side. It applies no decision rule and recommends no action.",
      formula: "A complete action-by-scenario matrix: rows are candidate actions, columns are scenarios, cells are outcomes. Every action must carry an identity and every cell must be present -- an incomplete matrix is refused, because a comparison across it would silently treat an unknown outcome as a known one. Scenario probabilities are carried only where the structure supplies them, and no expected value is computed without them.",
      abstain: "the governed decision structure `actionScenarioMatrix` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `actionScenarioMatrix` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "A wide spread across a row means that action's outcome depends heavily on which scenario occurs. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.5", name: "Decision Sensitivity Matrix", mc: "Decision_Sensitivity_Matrix",
      purpose: "Perturbs a declared decision model across a declared range and recomputes the ranking of alternatives, reporting the exact parameter values at which the ranking reverses.",
      formula: "A weighted-additive model over the declared criteria with one declared swept parameter. The difference between two alternatives' scores is linear in that parameter, so each crossover is solved exactly as a linear equation and reported as an exact fraction rather than detected by sampling a grid. No default weight and no default range is supplied: both arrive in the declared structure or the method reports nothing.",
      abstain: "the governed decision structure `decisionSensitivityModel` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `decisionSensitivityModel` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "A crossover inside the declared range means the preferred alternative depends on a parameter value that has not been settled; no crossover means the ranking is stable across the whole declared range. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.6", name: "Pareto Frontier Analysis", mc: "Pareto_Frontier",
      purpose: "Reports which of the declared alternatives lie on the Pareto frontier of the declared trade space and which are dominated, together with what dominates each dominated point.",
      formula: "The same dominance relation as B4.1, asked of the trade space. An alternative is on the frontier exactly when no other alternative dominates it, which does not depend on the order the alternatives were offered in. Identical objective vectors do not dominate one another, because dominance requires a strict improvement somewhere, so duplicate points both remain on the frontier.",
      abstain: "the governed decision structure `decisionAlternatives` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `decisionAlternatives` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "A single point is not a trade space and produces no frontier. The frontier is a property of the declared alternative set, not of the project. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
    ...
    { n: "B4.7", name: "Minimax Regret Decision Rule", mc: "Minimax_Regret_Decision_Rule",
      purpose: "Applies the minimax regret decision rule to a declared payoff or cost matrix over candidate actions and future states, reporting each action's maximum regret and which action minimises it.",
      formula: "Regret is the gap between an outcome and the best outcome available in the same future state. For a payoff matrix R(a,s) = max_a P(a,s) - P(a,s); for a cost matrix the best in a state is the minimum, so R(a,s) = P(a,s) - min_a P(a,s). The reported alternative is argmin over actions of max over states of R. The matrix's orientation is declared and never assumed. Where several actions share the lowest maximum regret, all of them are returned and none is chosen, because breaking the tie requires a preference that is not governed here.",
      abstain: "the governed decision structure `actionScenarioMatrix` is absent from the project, so the module returns Not Estimable rather than a reading.",
      sources: "the governed `actionScenarioMatrix` supplied through the project-data intake. The controlled document corpus does not carry a decision problem, so this module currently returns Not Estimable on real projects.",
      interp: "A low maximum regret means the action performs acceptably across every declared future, not that it is best in any of them. It names no action and authorises nothing: the row is an analytical result, it carries no status colour, it cannot reach the governed status, and the decision remains the reviewer's.",
  ...
  const PH_MODULES = [
    ...
    { n: "D1.4", name: "Cross-project Pattern Detector", mc: "Cross_Project_Pattern",
  ...
  const STATUS_RULES_TOPIC = {
    ...
    build: () => `
      ...
      <h3>Category status: two rules, by category</h3>
      <p>Category status is produced by one of two explicit rules, and which one depends on what the category measures. The four performance categories &mdash; Cost and EVM Performance, Schedule Performance, Cost Risk and Document-Derived Signals &mdash; <strong>average</strong> the scores of the modules that asserted a band: Green +2, Yellow +1, Amber &minus;1, Red &minus;2, averaged over only those modules, with Green at or above 1.5, Yellow at or above 0.5, Amber at or above &minus;0.5 and Red below it. One weak module among several moves the posture without dominating it. Delivery Quality Performance instead takes the <strong>worst</strong> band any of its modules asserted: quality, safety, environmental and contractor performance are conformance and compliance measures, and an adverse reading in one of them is a finding in its own right, so a single Amber makes the category Amber and a single Red makes it Red. A module that computed without a band, abstained or failed is in neither rule and is never counted as a zero; a category where no module banded carries no posture at all. A belief-combination conflict coefficient is still computed and stored beside the posture as a measure of how much the modules disagreed, but it no longer decides the posture.</p>

      <h3>Project status: which categories vote</h3>
      <p>Project status is the <strong>worst</strong> band among the categories that carry one, and only the categories that describe the condition of the project take part. It is withheld as Indeterminate unless every one of the five required categories carries a posture. Project Health and Recommendation and Governance vote. Data and Evidence Health does not: it measures how trustworthy the evidence base is, and early reporting periods carry the least evidence, so folding it in would make an early period read worse for reasons that have nothing to do with the project. Portfolio Level does not vote either: it needs more than one project and never runs on a single-project path.</p>
