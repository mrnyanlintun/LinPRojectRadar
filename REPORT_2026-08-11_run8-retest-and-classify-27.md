...
- **The full existing suite returns to green after every temporary mutation is restored.** VERIFIED. Server suite, fresh SQLite per file via `alembic upgrade head`, `PYTHONIOENCODING=utf-8` throughout, interpreter confirmed real by a successful `fastapi`, `sqlalchemy` and `alembic` import in the runner before any test file was executed. **64 files, 4,612/4,612 checks under the `RESULT: n/n` convention, plus `test_run5_export.py`'s own 34/34 which uses a different convention, so 4,646 in total, 0 failing files.** No existing check went red, nothing had to be re-pointed or loosened, and this run's file contributes all 233 of the increase over the Run 7 baseline.
- **Browser and render suites.** NOT RUN, and stated rather than skipped silently. None of the 27 depends on end-to-end rendering for anything this run classifies: every finding is server-side arithmetic, domain or boundary behaviour, and the abstention-to-ledger chain was asserted join by join and at the renderer by Run 6. `tests_render.html` never loads `index.html`, so a check asserting what index.html loads there would be vacuous. The browser-driving files inside the server suite (`test_theme_plain.py`, `test_map_and_module_count.py`, `test_disclaimers.py`, `test_training_detail.py`) ran as part of the 64 and are green.
